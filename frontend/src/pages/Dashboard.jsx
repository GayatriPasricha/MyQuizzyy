    ...
    <div className="flex-1 bg-slate-50 p-6 sm:p-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-black text-slate-800 mb-2">My Dashboard</h1>
        ...
        {quizzes.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl text-center border border-slate-100 shadow-sm">
            ...
            <h3 className="text-xl font-bold text-slate-700 mb-2">No quizzes yet</h3>
            <p className="text-slate-500 font-medium mb-6">You haven't generated any quizzes with AI yet.</p>
            <Link to="/" className="text-primary-600 font-bold hover:underline underline-offset-4">Generate your first one now!</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {quizzes.map((quiz, i) => (
              <motion.div 
                ...
                key={quiz._id} 
                className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xl shadow-slate-200/30 hover:border-primary-200 hover:-translate-y-1 transition-all group"
              >
                ...
                <h3 className="font-bold text-lg text-slate-800 mb-2 line-clamp-2">{quiz.title}</h3>
                ...
                <div className="pt-4 border-t border-slate-100 flex gap-3">
                  <Link to={`/quiz/${quiz.shortId}`} className="flex-1 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold py-2 rounded-lg text-center transition-colors border border-slate-200">
                    Take Quiz
                  </Link>
                  <button 
                    onClick={() => {
                    ...
                    className="p-2 border border-slate-200 rounded-lg text-slate-500 hover:text-primary-600 hover:border-primary-200 hover:bg-primary-50 transition-colors"
                  >
